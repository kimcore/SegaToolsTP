...
HRESULT idz_io_jvs_init(void)
{
    ...
    else if (wstr_ieq(idz_io_cfg.mode, L"tp")) {
        hr = idz_tp_init(&idz_io_cfg.tp, &idz_io_backend);
    }
    else {
        hr = E_INVALIDARG;
...
void idz_io_jvs_read_buttons(uint8_t *opbtn_out, uint8_t *gamebtn_out)
{
    ...
    if (*ffbOffset & 0x010000) {
        opbtn |= IDZ_IO_OPBTN_TEST;
    }

    if (*ffbOffset & 0x020000) {
...
void idz_io_jvs_read_analogs(struct idz_io_analog_state *out)
{
	...
	//printf("%d",((tmp.wheel * idz_io_cfg.restrict_) / 128));
